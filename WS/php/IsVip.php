    ...
    <?php
        if($logeatuta){
            if ($session_mota == "irakasle") {
                echo "<h2>VIPa egiaztatzeko REST bezeroa</h2><br>";
                echo "<h3>Adierazi egiaztatu nahi duzun VIParen posta:</h3><br>";
                echo '<form method="POST" action="../php/IsVip.php">
                          <label for="posta">Ikaslearen posta: </label>
                          <input type="text" name="posta" id="posta"/>
                          <input type="submit" name="submit" id="submit" value="VIPa da?"/>
                      </form>';

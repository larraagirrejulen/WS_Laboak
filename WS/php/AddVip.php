...
<body>
    ...
    if($logeatuta){
        if ($session_mota == "irakasle") {
            echo "<h2>VIPa txertatzeko REST bezeroa</h2><br>";
            echo "<h3>Adierazi txertatu nahi duzun VIParen posta:</h3><br>";
            echo '<form method="POST" action="../php/AddVip.php">
                      <label for="posta">Ikaslearen posta: </label>
                      <input type="text" name="posta" id="posta"/>
                      <input type="submit" name="submit" id="submit" value="VIP berria txertatu"/>
                  </form>';
            ...
            if(isset($_POST['posta'])){
                ...
                $data = array('posta' => $jasotakoPosta);
